...
def view_password(passwords, key):
    ...
    for i, (service, username, password) in enumerate(passwords):
        if service == service_name:
            ...
            break
    if not found:
        print(Fore.RED + f"Password for service '{service_name}' not found.")

    # Wait for Enter key press before returning to the main menu
...
def list_services(passwords):
    if not passwords:
        print(Fore.YELLOW + "No services saved.")
    else:
...
def main():
    ...
    if choice in ["yes", "y"]:
        ...
        print(Fore.CYAN + "\nGenerated your secret key in the current folder (secret.key). KEEP IT SAFE, it's used to retrieve your passwords!")
        generate_key()
        key = load_key()
    else:
    ...
    while True:
        ...
        print("2. View password for a service")
        print("3. Change password for a service")
        print("4. View list of saved services")
        print("5. Exit")
